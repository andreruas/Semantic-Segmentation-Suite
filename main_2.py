...
####################################################################################################################################
        ...
        # Create directories if needed
        if not os.path.isdir("%s"%("checkpoints_2/Val")):
                os.makedirs("%s"%("checkpoints_2/Val"))

        target=open("%s/train_scores.csv"%("checkpoints_2/Val"),'a')
        if (epoch == 0):
...
##############################################################################################################################
        ...
        if 1:
            ...
            target_avg=open("%s/val_scores.csv"%("checkpoints_2/Val"),'a')
            if (epoch == 0):
                target_avg.write("epoch, avg_score, avg_precision, avg_recall, avg_f1, avg_iou %s\n" % (class_names_string))

            target=open("%s/%04d/val_scores.csv"%("checkpoints_2",epoch),'a')
